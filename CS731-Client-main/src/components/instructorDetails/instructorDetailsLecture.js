...
const LecturesSection = ({ lectures, participants, onAttendanceChange, onSaveAttendance, onAddLecture }) => {
  // Function to check if today's date is in the lectures list
  const isTodayLecturePresent = () => {
    const today = new Date().toLocaleDateString();
    return lectures.some(lecture => new Date(lecture.lectureDate).toLocaleDateString() === today);
  };
  ...
  return (
    <Box mt={3} sx={{ textAlign: 'left', width: '100%' }}>
      <Button
        ...
        disabled={isTodayLecturePresent()} // Disable button if today's date is present
                    ...
                    <Box display="flex" alignItems="center" sx={{ width: '100%' }}>
                      ...
                      <FormControl sx={{ minWidth: 120, ml: 3 }}>
                        ...
                        >
                          ...
                          <MenuItem value="Tardy">Late</MenuItem>
